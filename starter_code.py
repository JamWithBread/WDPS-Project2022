...
To run code normally:
    python3 starter_code.py {path_to_data} 

To run code as quickly as possible (recommended):
    python3 starter_code.py {path_to_data}} --multi True

To run code and monitor the progress of the program and see Elasticseach errors:
    python3 starter_code.py {path_to_data} --verbose True

"""

##########################################################################################################
####### Before running, run 'bash requirements.txt' in container cmd line to install dependencies ########
##########################################################################################################
...
def process_record(i, record, verbose):
    ...
    if verbose:
        print(f"\n--Finding relations for doc #{i}")

    relations = find_relations(text, ent_dict, key)

    if verbose:
        print(f"--Entity Linking and Relation Extraction finished for doc #{i} - Total Time: {round(time.time()-t00,2)}s\n")
...
if __name__ == '__main__':
    try:
        ...
        parser.add_argument('--multi','--m', help = "use '-m True' or '--multi True' to run program on multiple threads", default = False)
